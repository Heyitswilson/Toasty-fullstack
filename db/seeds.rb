...
product1 = Product.create!({name: 'Eggplant Bag', description: "Honestly, why wouldn't you want this?", price: 25, artist_id: User.find_by(name: 'Wilson').id, keep: true})
product2 = Product.create!({name: "Wooden Wall Map", description: "It's the perfect piece for when you want to see the world but don't have time!", price: 70, artist_id: User.find_by(name: 'Wilson').id, keep: true})
...
price: 40, artist_id: User.find_by(name: 'Wilson').id, keep: true})
...
price: 13.50, artist_id: User.find_by(name: 'May').id, keep: true})
product5 = Product.create!({name: "Schnauzer Succulent Planter, Shiba Inu Animal Planter, Corgi Gifts, Sausage Dog Planter, Akita Pug Gift Dachshund Gifts,Siberian Husky Gifts", description: "Yes, you really do get all of them. Ideal for succulents, air plants, and cacti. 
    There is a drainage hole in every planter. Plants are not included.", price: 60, artist_id: User.find_by(name: 'May').id, keep: true})
product6 = Product.create!({name: "Botanical Print Set", description: "Frames are included!", 
price: 40, artist_id: User.find_by(name: "Ryan").id, keep: true})
product7 = Product.create!({name: "Surfing Pikachu Postcard", description: "Gotta catch 'em all!", price: 10, artist_id: User.find_by(name: "Wilson").id, keep: true})
product8 = Product.create!({name: 'HORTUM 10ø | Concrete Planter - Plant Pot - Diameter 10cm (4 inch)', description: "Perfect for your houseplants! There is cork protection at the base to prevent scratches and the surface is sealed with a non-toxic product to make it waterproof.", price: 45, artist_id: User.find_by(name: "Melissa").id, keep: true})
product9 = Product.create!({name: 'Glasses Succulent Planter', description: "Brighten up your room with this super cute succulent planter!", price: 13, artist_id: User.find_by(name: "Melissa").id, keep: true})
product10 = Product.create!({name: 'Bright Color Brush Strokes Contemporary Artwork Extra Large Square Colorful Modern Abstract Oversize Wall Art Hand Made Canvas Oil Painting', description: "I really this painting! Check out 'EARTITALY' on Etsy to find more of their artwork!", price: 700, artist_id: User.find_by(name: "Priscilla").id, keep: true})
product11 = Product.create!({name: 'Toast', description: "I asked for toast and got this drawn and sent to me within two minutes. Shout-out to Karissa!", price: 999, artist_id: User.find_by(name: "Karissa").id, keep: true})
